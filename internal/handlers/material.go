...
func GetMaterials() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 3. 关键字搜索（不限定学期）
		if keyword != "" {
			list, err := models.SearchMaterials(database.DB, keyword)
			if err != nil {
		...
		// 4. 全部教材
		list, err := models.GetAllMaterials(database.DB)
		if err != nil {
...
func CreateMaterial() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if req.Semester != "" && len(req.ClassNames) > 0 {
			...
			if _, err := models.ImportMaterialsFromRows(database.DB, rows, req.Semester, req.AcademicYear); err != nil {
...
func ImportMaterialsExcel() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		inserted, err := models.ImportMaterialsFromRows(database.DB, rows, semester, academicYear)
		if err != nil {
